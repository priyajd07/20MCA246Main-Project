...
def upload_img():
    ...
    img.save('static/uploads/'+fname)
    res=prep('static/uploads/'+fname)
    print(res)
    if(res[0]=='1'):
        return '''<script>alert('autistic');window.location='/uploadimage'</script>'''
    else:
        return '''<script>alert('not autistic');window.location='/uploadimage'</script>'''
